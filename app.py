# -*- coding: utf-8 -*-
"""
Created on Tue Mar 25 15:07:04 2025
...
# ==== Load raw data ====
raw_data = pd.read_csv("C:/Users/eulal/Documents/project 1 python/testData_2019_Civil - testData_2019_Civil.csv")
raw_data.rename(columns={"Civil (kWh)": "Power_kW", "Date": "Timestamp"}, inplace=True)
raw_data["Timestamp"] = pd.to_datetime(raw_data["Timestamp"])

# ==== Load forecast data ====
forecast_df = pd.read_csv("C:/Users/eulal/Documents/project 1 python/forecast_jan_mar_2019.csv")
...
app.layout = html.Div([
   ...
   dcc.Tabs(id="tabs", value='tab1', children=[
    ...
    dcc.Tab(label='Regression & Metrics', value='tab5')  # NOVO
]),
...
def render_tab(tab):
    if tab == 'tab1':
        fig = px.line(raw_data, x='Timestamp', y='Power_kW',
                      title="Energy Consumption (2019)")
        return html.Div([
        ...
        return html.Div([
            ...
            html.Ul([
                html.Li(f"NMBE: {nmbe:.2f}%"),
                html.Li(f"cvRMSE: {cvrmse:.2f}%"),
...
fig.add_shape(type="line", x0=min(y_test), y0=min(y_test), x1=max(y_test), y1=max(y_test), line=dict(color='green'))

# (6) Calcula métricas escolhidas
metrics_text = []
